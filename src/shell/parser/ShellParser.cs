...
public enum ParseState { SPACE, WORD, WEAK_Q, STRONG_Q, VARIABLE }
public class ParseAutomaton {
    ...
    public void Reduce(Token top) {

        //FIXME: remove 
        // Console.WriteLine("----");
        // Console.WriteLine($"+ {top.ToDebugString()}");
        Token prev = top;
        Token? next = top;
        while (next != null) {
...
public class ShellParser {
    public Result<Expression> Parse(string? input, ShellEnvironment env) {
        ...
        if (result.Type == TokenType.T_TMP) {
            ...
            return this.Parse(
                tmp.Resolve(tmp.VarNames.Select(x => {
                    var r = env[x];
                    #if DEBUG
