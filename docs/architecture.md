...
### 1) App
Класс приложения. Основной класс, именно в нём находится метод `Main` - точка входа в программу, управляет `UI`, `Interpreter`. 
...
#### 1.1) Main
Метод мейн - точка входа в программу. В нём происходит инициализация `UI` и `Interpreter` и запуск основного цикла работы программы.
...
#### 3.1) ParseAutomaton
ParseAutomaton транслирует пользовательский ввод в стек токенов, из которых парсер составит выражение (`Expression`).  
#### 3.2) Token (Токен)
...
## Детали работы этапов программы

### 1) Парсинг и объектное представление результата разбора.
Работа парсера:
+ `ShellParser` (Парсер) получает пользовательский ввод, и оркужение, инициализирует `ParseAutomaton` 
+ `ParseAutomaton` транслирует пользовательский ввод в стек токенов
+ Парсер пытается сконструировать `Expression` из стека токенов и передаёт интерпретатору обёрнутый в монаду `Result` результат.

 [Выражение](Архитектура.md#Expression) - класс, получаемый после парсинга, описывает набор действий, которые нужно исполнить. Как было описано имеет 3 представителя присваивание, команды и пайп. Выражения реализуют метод `Run`, конструирующий список объектов для исполнения - `Box`.
 
### 2) Подстановка
 Подстановку выполняет парсер, если возникает соответствующий токен (`TemplateToken`).  Токен конструируется `ParseAutomaton` и имеет метод `Resolve` который по переданным аргументам подставляет заместо переменных в строке значения.
 
 После подстановки парсер переобрабатывает строку заново.
 
### 3) Аргументы и входной поток
Само исполнение команды заложено в метод `Run`, которому передаётся массив строк - аргументы команды. То есть само исполнение всегда исполняется от аргументов. Инкапсуляция исполнения в класс `Box` позволяет и хранение командой поля потока ввода, реализовывает работу с потоками, при этом абстрагирует итоговое исполнение самой команды от них.    
